...
class GeneratorFile {
    ...
    toJSON() {
        ...
        const filename = new StringTransformer_1.StringTransformer(path_1.basename(this.name))
            .dropExtension()
            ...
            .toValue();
        const initialFilePath = this.name.replace(path_1.basename(this.name), filename);
        ...
        const filepath = this.customDestinationPath
            ? (path_1.isAbsolute(this.customDestinationPath)
                ? path_1.join(this.customDestinationPath, initialFilePath)
                : path_1.join(appRoot, this.customDestinationPath, initialFilePath))
            : path_1.join(appRoot, initialFilePath);
        /**
        ...
        const contents = this.stubContents
            ? (this.isStubRaw
                ? template_1.template(this.stubContents, templateContents, undefined, this.mustache)
                : template_1.templateFromFile(this.stubContents, templateContents, this.mustache))
            : '';
        return {
            filename,
